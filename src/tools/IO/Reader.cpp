...
bool Reader::Initialise(std::string configfile, DataModel &data)
{
    ...
    if (!m_variables.Get("Verbosity", m_verbosity))
        m_verbosity = 0;
    if (!m_variables.Get("Reader_Receive_Wait", Reader_Receive_Wait))
        Reader_Receive_Wait = 100;
    if (!m_variables.Get("Reader_Quit_Wait", Reader_Quit_Wait))
        Reader_Quit_Wait = 1000;
    if (!m_variables.Get("Thread_Send_Wait", Thread_Send_Wait))
        Thread_Send_Wait = 200;
    if (!m_variables.Get("Data_Batch_Size", Data_Batch_Size))
        Data_Batch_Size = 100;
